...
class BERTTool(object):
    def init(args):
        BERTTool.bert = BartModel.from_pretrained(args.bert.location)
        BERTTool.tokenizer = BartTokenizer.from_pretrained(args.bert.location)
...
class Model(models.KBRetriever_DC.base.Model):
    ...
    def get_info(self, batch):
        ...
        tokenized = self.tokenizer(construced_infos, last_responses, truncation='only_first', padding=True, return_tensors='pt', max_length = self.tokenizer.max_model_input_sizes['facebook/bart-base'], return_token_type_ids=True)
        tokenized = tokenized.data
        return tokenized['input_ids'].to(self.device), tokenized['token_type_ids'].to(self.device), tokenized['attention_mask'].to(self.device)
    ...
    def forward(self, batch):
        ...
        loss = torch.Tensor([0])
        if self.training:
            loss = F.cross_entropy(out_qi, torch.Tensor(utils.tool.in_each(batch, lambda x : x["consistency"][0])).long().to(self.device))\
                   + F.cross_entropy(out_hi, torch.Tensor(utils.tool.in_each(batch, lambda x : x["consistency"][1])).long().to(self.device))\
                    + F.cross_entropy(out_kbi, torch.Tensor(utils.tool.in_each(batch, lambda x : x["consistency"][2])).long().to(self.device))
        out = []
