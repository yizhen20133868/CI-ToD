...
from transformers import LongformerTokenizer, LongformerModel, AdamW
...
class BERTTool(object):
    def init(args):
        BERTTool.bert = LongformerModel.from_pretrained(args.bert.location)
        BERTTool.tokenizer = LongformerTokenizer.from_pretrained(args.bert.location)
...
class Model(models.KBRetriever_DC.base.Model):
    ...
    def get_info(self, batch):
        ...
        tokenized = self.tokenizer(construced_infos, last_responses, truncation='only_first', padding=True,
                                   return_tensors='pt',
                                   max_length=self.tokenizer.max_model_input_sizes['allenai/longformer-base-4096'],return_token_type_ids=True )
